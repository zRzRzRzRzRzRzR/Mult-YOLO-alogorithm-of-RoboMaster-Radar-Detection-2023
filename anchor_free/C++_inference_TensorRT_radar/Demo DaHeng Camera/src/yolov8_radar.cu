...
#define INFOF(...)            __log_func(__FILE__, __LINE__, LogLevel::Fatal, __VA_ARGS__)
    ...
    static bool check_runtime(cudaError_t e, const char *call, int line, const char *file) {
        if (e != cudaSuccess) {
            INFOE("CUDA Runtime error %s # %s, code = %s [ %d ] in file %s:%d", call, cudaGetErrorString(e),
...
#define TRT_VERSION_STRING(major, minor, patch, build)   TRT_STR(major) "." TRT_STR(minor) "." TRT_STR(patch) "." TRT_STR(build)
    ...
    void set_device(int device_id) { //设置显卡编号
        if (device_id == -1)
            return;
        checkCudaRuntime(cudaSetDevice(device_id));
    }
    ...
    warp_affine_bilinear_and_normalize_plane_kernel(uint8_t *src, int src_line_size, int src_width, int src_height,
...
//        printf("position=%d,newpositon=%d\n",position,position/num_elements+(position%num_elements)*num_bboxes);
    }
    ...
    */
    ...
    public:
        ...
        explicit Tensor(int n, int c, int h, int w, std::shared_ptr<MixMemory> data = nullptr,
                        int device_id = CURRENT_DEVICE_ID);

        explicit Tensor(int ndims, const int *dims, std::shared_ptr<MixMemory> data = nullptr,
                        int device_id = CURRENT_DEVICE_ID);

        explicit Tensor(const std::vector<int> &dims, std::shared_ptr<MixMemory> data = nullptr,
                        int device_id = CURRENT_DEVICE_ID);
        ...
        Tensor &set_stream(cudaStream_t stream) {
            stream_ = stream;
            return *this;
        }
    ...
    Tensor &Tensor::copy_from_gpu(size_t offset, const void *src, size_t num_element, int device_id) {
        ...
        if (head_ == DataHead::Device) {
            ...
            if (current_device_id != gpu_device_id) {
                checkCudaRuntime(cudaMemcpyPeerAsync(gpu<unsigned char>() + offset_location, gpu_device_id, src,
                ...
                checkCudaRuntime(cudaMemcpyAsync(gpu<unsigned char>() + offset_location, src, copyed_bytes,
                                                 cudaMemcpyDeviceToDevice, stream_));
            }
        } else if (head_ == DataHead::Host) {
            AutoDevice auto_device_exchange(this->device());
            checkCudaRuntime(
                    cudaMemcpyAsync(cpu<unsigned char>() + offset_location, src, copyed_bytes, cudaMemcpyDeviceToHost,
                                    stream_));
        } else {
    ...
    std::shared_ptr<std::vector<uint8_t>> TRTInferImpl::serial_engine() {
        ...
        auto output = make_shared<std::vector<uint8_t>>((uint8_t *) memory->data(),
    ...
    struct AffineMatrix { //将图片转化为推理格式
    ...
    public:
        ...
        virtual bool
        startup(const string &file, Type type, int gpuid, float confidence_threshold, float nms_threshold) {

            if (type == Type::V7 || type == Type::V8) {
                normalize_ = Norm::alpha_beta(1 / 255.0f, 0.0f, ChannelType::SwapRB); // 归一化
            } else {
        ...
        virtual void worker(promise<bool> &result) override {
            ...
            set_device(gpuid); //设置GPU_ID
            auto engine = load_infer(file); //读取引擎。
            if (engine == nullptr) { //如果没有载入网络
                INFOE("Engine %s load failed", file.c_str());
                result.set_value(false); //返回错误
                return;
            }
            engine->print(); //打印引擎信息
            const int MAX_IMAGE_BBOX = 1024;   // 最大的单图BBOX数量(默认1024）
            const int NUM_BOX_ELEMENT = 7;      // left, top, right, bottom, confidence, class, keepflag
            Tensor affin_matrix_device;  //旋转后读入的图片tensor
            Tensor output_array_device;  //推理的输出tensor
            int max_batch_size = engine->get_max_batch_size();
            auto input = engine->tensor("images");
            auto output = engine->tensor("output");
            int num_classes = output->size(1) - 4; //类别

            input_width_ = input->size(3);
            input_height_ = input->size(2);
            tensor_allocator_ = make_shared<MonopolyAllocator < Tensor>>
            (max_batch_size * 2);
            stream_ = engine->get_stream();
            gpu_ = gpuid;
            result.set_value(true); //返回Tensor数据

            input->resize_single_dim(0, max_batch_size).to_gpu(); //发送维度数据
            ...
            while (get_jobs_and_wait(fetch_jobs, max_batch_size)) {
                ...
                for (int ibatch = 0; ibatch < infer_batch_size; ++ibatch) {
                    ...
                    decode_kernel_invoker(pre, output->size(2), num_classes, confidence_threshold_,
                                          nms_threshold_, affine_matrix, output_array_ptr, MAX_IMAGE_BBOX, stream_);
                }
                ...
                for (int ibatch = 0; ibatch < infer_batch_size; ++ibatch) {
                    ...
                    for (int i = 0; i < count; ++i) {
                        ...
                        Box.x = pbox[0];
                        Box.y = pbox[1];
                        Box.width = pbox[2] - pbox[0];
                        Box.height = pbox[3] - pbox[1];
                        float prob = pbox[4];
                        int label = pbox[5];
                        int keepflag = pbox[6];
                        if (keepflag == 1) {
        ...
        virtual vector<shared_future<ObjectArray>> commits(const vector<Mat> &images) override {
            return ThreadSafedAsyncInferImpl::commits(images);
        }

        virtual std::shared_future<ObjectArray> commit(const Mat &image) override {
    ...
    void image_to_tensor(const cv::Mat &image, shared_ptr<Tensor> &tensor, Type type, int ibatch) {
        ...
        checkCudaRuntime(
                cudaMemcpyAsync(affine_matrix_device, affine_matrix_host, sizeof(affine.d2i), cudaMemcpyHostToDevice,
                                stream));
